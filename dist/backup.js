...
if (accessKeyID && secretAccessKey && bucketName) {
    S3Client = new aws_sdk_1.S3({ region, credentials: new aws_sdk_1.Credentials(accessKeyID, secretAccessKey) });
}
...
function markFileAsLatest(fileName) {
    S3Client.upload({
        ...
        if (err) {
            ...
            console.log(`File ${fileName} has been marked as latest in bucket ${bucketName}`);
        }
    });
}
